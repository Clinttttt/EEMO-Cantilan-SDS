...
**Status:** Planning approved; Phases 0, 1 and 2A are complete. Phase 2B.1 (TPM) and 2B.2 (TRM) are implemented and reviewed as shadow-only work, with PRs #9 and #10 open and integration pending; Phase 2 remains in progress. After both shadow slices are integrated, the planned next milestone is Phase 3 — AccountableDocument / OR pilot.
...
| Phase 2 — Collection Ledger | **IN PROGRESS. Phase 2A foundation is COMPLETE.** `Collection` and `CollectionLine` exist as a dormant, tenant-scoped, auditable, backup/restorable ledger foundation. Phase 2B.1 TPM (PR #9) and 2B.2 TRM (PR #10) are implemented, reviewed shadow-only slices with integration pending. They prove bounded source mapping without writing ledger rows or switching current writers/reports. Phase 2 is not complete and no production cutover has occurred. |
| Phase 3 — AccountableDocument / OR pilot | Planned next bounded milestone after both Phase 2B shadow proofs are integrated and verified. Establish OR ownership, document history/replacement and compatible itemized lines for a bounded OR workflow. Reconcile existing OR registry and legacy fields before switching writers. |
...
**Phase 2B.1 — TPM shadow reconciliation:** implemented and locally verified, independently reviewed; PR #9 is open with integration pending. It projects eligible paid `TpmAttendance` money to the `TABO` semantic classification and effective policy without writing `Collection` or `CollectionLine`.

**Phase 2B.2 — TRM shadow reconciliation:** implemented and locally verified, independently reviewed; PR #10 is open with integration pending and is stacked on PR #9. It projects persisted `TrmTrip.Fee` using the Philippine-local date derived from `RecordedAt` to `TRANSPORTATION_PARKING`, without inferring a vehicle class, changing historical OR evidence, or writing ledger rows.

Neither shadow slice is merged, deployed or production-verified. Both preserve their specialized source as authoritative. No production `Collection`/`CollectionLine` writer or classified report has cut over; Phase 2 remains in progress.

**Planned next implementation milestone:** after both shadow PRs are integrated and their checks are verified, proceed toward the bounded **Phase 3 — AccountableDocument / OR pilot**. TPM demonstrates source/classification mapping from a `DateOnly` activity source; TRM demonstrates mapping from a UTC timestamp through the Philippine business date. Together they exercise tenant-safe policy resolution and explicit unresolved-money accounting without replacing either source. This is readiness for the next bounded pilot, not a declaration that Phase 2 is complete.
