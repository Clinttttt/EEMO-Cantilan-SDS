...
- The older 3+-months/1–2-months rule recorded in `EEMO_Complete_Documentation.md` is superseded as a business assumption. Current production now applies the one-fully-elapsed-unpaid-month delinquency threshold; one to two months remain lower-age/Normal follow-up and three-plus months higher-age/Critical follow-up. This runtime change does **not** resolve the separate old/lapsed Arrears qualification boundary or complete the later Arrears recovery/reporting cutover.
...
| Phase 5B — delinquency/Arrears transition | **PARTIALLY COMPLETE.** Current production now applies the one-month delinquency threshold and separates 1–2 month Normal from 3+ month Critical urgency. The old/lapsed Arrears qualification boundary, qualifying recovery classification/application and separate Monthly Income presentation gate remain unresolved before Arrears report cutover. |
...
Phase 2A has not introduced `CollectionAllocation`, `ReceivableObligation`, `AccountableDocument`, OR migration, Cash Ticket inventory/issuance, classified RCD, classified Monthly Income, installment migration, full Arrears qualification/recovery cutover, or revenue-target setup. The one-month delinquency runtime rule has since been deployed separately; it does not make the unresolved Arrears boundary complete. Existing `PaymentRecord`, `DailyCollection`, `UtilityBill`, `SlaughterTransaction`, `TpmAttendance`, `TrmTrip`, `OnlinePaymentTransaction`, and existing OR/reporting flows remain authoritative/current behavior. No current production money writer or production report consumes `Collection`/`CollectionLine` yet.

## 14. Business decision gates

| Decision gate | Blocks or constrains |
|---|---|
| Exact old/lapsed Arrears qualification boundary | Determines which debts qualify for the confirmed Arrears recovery classification and blocks the remaining Arrears status/recovery/reporting cutover. The one-month Delinquent runtime threshold is already deployed. |
...
| Currentness of Ordinance No. 12-2021 / any superseding transport schedule | Final production transportation/parking catalog and rate rollout in Phases 6–7. The ordinance supplies a concrete class/rate baseline, but production must not assume it is still unamended. |
...
| Annual-target authoritative source, period, approval/governance, scope and revision policy | Production Revenue Target Setup and attainment semantics in Phase 9. Existing Monthly Income material proves target-report precedent, not editing authority. |
