...
> behind the design see `../architecture/SYSTEM_ARCHITECTURE.md`, and for code shapes to copy see `../architecture/APPLICATION_PATTERNS.md`. When those
...
- Platform-operator powers come only from the `IsPlatformOperator` flag. A municipality Head is not a platform
  operator, including the default municipality's Head. Cross-tenant work requires the dedicated operator authority.
...
- **Two-factor (TOTP)** is currently optional for supported accounts. A Head without MFA receives one dismissible,
  server-recorded reminder after sign-in; enrollment is not an authorization gate. RFC 6238 over `HMACSHA1`, verified
  against the RFC's published vectors; secrets encrypted with `ICredentialProtector` (AES-256-GCM); 8 single-use
...
Assessment request (public form)
  ...
  → Head sets their password; the portal opens as that LGU and offers optional two-factor enrollment
```
