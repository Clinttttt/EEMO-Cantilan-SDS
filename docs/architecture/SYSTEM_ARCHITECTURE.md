...
Companion to `ARCHITECTURE_RULES.md` (what is allowed) and `APPLICATION_PATTERNS.md` (what to copy). This file explains the
...
`../business/REVENUE_ARCHITECTURE.md`. This document continues to explain the shape and trade-offs of the current system.
...
- **Enrollment is currently optional.** A Head without MFA receives a one-time, dismissible reminder after sign-in;
  the reminder is recorded server-side and is not an authorization gate.
- **For an MFA-enabled account, the challenge is the credential** for the second step: the password step issues a
  hashed, single-use, 5-minute challenge and no tokens, so a correct password alone yields no session.
...
- A Head may clear their own factor after re-entering their password. Peer Heads remain blocked from each other's
  accounts; the dedicated platform operator provides the cross-tenant rescue path.
