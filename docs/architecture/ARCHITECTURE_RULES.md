...
Repository-wide implementation boundaries. Current accepted semantics are recorded in `../business/EEMO_BUSINESS_RULES.md`
and explicit current rulings. The approved target for the broader EEMO revenue expansion is
`../business/REVENUE_ARCHITECTURE.md`; it does not claim that target behavior is already implemented. When sources disagree,
...
- Guards, not ad-hoc checks: `AdminManagementGuard` (no Head may act on a peer Head),
  `PlatformOperatorGuard.IsCurrentAsync` and `PlatformOperatorGuard.IsDedicatedOperatorAsync` (both require the dedicated
  `IsPlatformOperator` flag; no municipality Head is a platform operator).
- Guards fail **closed**.
