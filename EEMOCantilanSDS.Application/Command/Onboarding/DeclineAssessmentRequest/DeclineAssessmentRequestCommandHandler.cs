...
{
    public class DeclineAssessmentRequestCommandHandler(IAppDbContext context, ICurrentUserService currentUser)
        : IRequestHandler<DeclineAssessmentRequestCommand, Result<AssessmentRequestDto>>
    {
        public async Task<Result<AssessmentRequestDto>> Handle(DeclineAssessmentRequestCommand request, CancellationToken ct)
        {
            ...
            if (entity.Status != AssessmentRequestStatus.PendingReview)
                return Result<AssessmentRequestDto>.Failure("Only a pending request can be declined.");

            entity.Decline(request.DecisionMessage, currentUser.Username ?? "Operator");
