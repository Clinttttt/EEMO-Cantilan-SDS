...
public class DashboardRepositoryTests : RepositoryTestBase
{
    ...
    public async Task GetOverview_ReportsRealCollected_Unpaid_Recent_AndDelinquents()
    {
        ...
        // Both stalls are behind for every month of their account that has closed: the contracts run from January
        // 2025 and the anchor is June 2026, so January 2025 – May 2026 is seventeen months. An NPM month is settled
        // when its rent is met, and neither stall has a daily collection or a fully-paid record for any of them
        // (the one Paid record is June 2026, the anchor month, excluded as still in progress). This is the figure
        // the register and the Follow-up History state for the same account — the count used to stop at twelve
        // months, which quietly reported ₱9,900 where ₱33,300 was owed.
        ...
        Assert.Equal(17, delinquent.MonthsUnpaid);
    }
    ...
    [Fact]
    public async Task GetOverview_CollapsesWholeMonthNpmDailyCollections_EvenWithNoOrNumber()
    {
        var context = NewContext();
        var npm = Facility.Create(FacilityCode.NPM, "New Public Market", "NPM");
        var stall = Stall.Create(npm.Id, "1", 900m, ApplicableFees.DailyRental);
        var contract = Contract.Create(stall.Id, "Joel Maligsa", null, new DateOnly(2025, 1, 1), 3, 900m);
        context.AddRange(npm, stall, contract);

        // Whole-month settlement recorded WITHOUT an OR (blank): same stall, same recording moment →
        // still collapses into one transaction row (grouped on stall + recording minute).
        ...
        var npmRows = overview.RecentTransactions.Where(t => t.FacilityCode == FacilityCode.NPM).ToList();
        var grouped = Assert.Single(npmRows);      // 4 blank-OR days → ONE row
        Assert.Equal(120m, grouped.Amount);         // 4 × ₱30
        Assert.Equal("Joel Maligsa", grouped.PayorName);
    }
